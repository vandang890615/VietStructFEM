...
"""
Steel Deck Module
Refactored from original main_window.py
"""
...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QGroupBox, QFormLayout,
                             ...
                             QTextBrowser, QMessageBox, QSplitter, QScrollArea,
                             QCheckBox)
...
try:
    ...
    HAS_WEBENGINE = False
    print("[WARNING] PyQtWebEngine not available. Plotly diagrams will not display.")
...
# Wind zones
try:
    from steeldeckfem.core.wind_zones import WIND_ZONES, CITY_WIND_ZONES, get_wind_pressure
except ImportError:
    WIND_ZONES = {"Vùng I - Nội địa": {"zone": "I", "Wo": 95}}
...
class FEMAnalysisThread(QThread):
    ...
    def run(self):
        try:
            ...
            results = analyzer.run_analysis(layout=self.layout)
...
class SteelDeckModule(QWidget):
    """
    Module: Steel Deck Floor System Calculator
    """
    ...
    def init_ui(self):
        ...
        # Set initial sizes to ensure input is visible comfortably
        splitter.setCollapsible(0, False) # Prevent hiding input
        splitter.setStretchFactor(0, 0) # Fixed/Minimum size policy for input
        splitter.setStretchFactor(1, 1) # Output takes remaining space
    ...
    def create_input_panel(self):
        ...
        scroll.setMinimumWidth(450) # Fix: Prevent layout from being too narrow
        panel = QWidget()
    ...
    def create_output_panel(self):
        ...
        self.tab_3d = QWidget()
        self.init_3d_tab()
        self.tabs.addTab(self.tab_3d, "🎨 Mô hình 3D")
        
        # Tab 2: FEM Results
        self.tab_fem = QWidget()
        self.init_fem_tab()
        self.tabs.addTab(self.tab_fem, "🔬 Kết quả FEM")
        ...
        # Add button to refresh report
        btn_regen = QPushButton("🔄 Cập nhật Thuyết minh")
        btn_regen.clicked.connect(self.generate_report)
        layout_report.addWidget(btn_regen)
        
        self.tabs.addTab(self.tab_report, "📝 Thuyết minh Tính toán")

        # Tab 4: Plotly Diagrams
        if HAS_ADVANCED_FEATURES:
            self.tab_plotly = QWidget()
            self.init_plotly_tab()
            self.tabs.addTab(self.tab_plotly, "📊 Biểu đồ Interactive")
        
        layout.addWidget(self.tabs)
        
        # Import Generator
        try:
            from steeldeckfem.core.steel_deck_report_generator import SteelDeckReportGenerator
            self.report_generator = SteelDeckReportGenerator
        except ImportError:
            self.report_generator = None
            print("Warning: Could not import SteelDeckReportGenerator")
    ...
    def generate_report(self):
        ...
        if not self.report_generator:
            QMessageBox.warning(self, "Lỗi", "Module tạo báo cáo chưa được tải.")
            return

        try:
            # Gather simplified params for demo
            params = {
                'L_beam': float(self.inp_col_x.text()) if self.inp_col_x.text() else 6.0, 
                'q_load': float(self.inp_wind.text() if hasattr(self, 'inp_wind') else 100) / 100 + 10, # Dummy load logic
            ...
            path = self.report_generator.generate_report(params)
